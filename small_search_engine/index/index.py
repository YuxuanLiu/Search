...
class Indexer(object):
    def __init__(self):
        ...
        self.magnitude = {}                 #{doc1:4.5, doc2: 5.5}  #magnitude is the squre of the norm
    ...
    def parse(self,raw_content, title):
        raw_content.encode("utf-8")
        terms = re.split("[^a-zA-Z0-9]+",raw_content)
        title_terms = re.split("[^a-zA-Z0-9]+",title)
    ...
    def load_data(self, pathname):
        ...
        for i in range(len(data)):
            ...
            item = self.get_content(filepath)
            if item != None:
                self.parse(item[0], item[1])
    ...
    def build_magnitutide(self):
    	for termID in self.postingLists:
    		for docID in self.postingLists[termID]:
    			if docID not in self.magnitude:
    				self.magnitude[docID] = 0
    			self.magnitude[docID] += self.postingLists[termID][docID] ** 2
    ...
    def write_magnitude(self):
        with open('magnitude.json', 'w') as f:
            json.dump(self.termId2docFre, f)
